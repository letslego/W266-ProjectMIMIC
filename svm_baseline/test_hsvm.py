...
parser.add_argument('-output_predictions_file', default='hsvm_predictions.data', help='File to save predictions to.')
parser.add_argument('-classifier_directory', default='hsvm_model/', help='Directory trained SVMs were saved to.')
...
for i in range(len(predictions)):
    ...
    for prediction in doc_predictions:
        if prediction == root or np.all([anc in doc_predictions for anc in ancestors[prediction]]):
            filtered_predictions.append(prediction)
    predictions[i] = filtered_predictions
